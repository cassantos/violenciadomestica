...
def make_choropleth(input_df, input_id, input_column, input_color_theme):
    choropleth = px.choropleth(input_df, locations=input_id, color=input_column, locationmode="USA-states",
                               ...
                               range_color=(0, max(df_violencia_estado.SofreuViolencia)),
                               #scope="brazil",      
                               scope="usa",                   
...
with col[1]:
    ...
    choropleth = make_choropleth(df_violencia_estado, 'Estado', 'SofreuVioneica', selected_color_theme)
    st.plotly_chart(choropleth, use_container_width=True)
    
    heatmap = make_heatmap(df, 'Ano', 'Estado', 'Sofreu Violência', selected_color_theme)
